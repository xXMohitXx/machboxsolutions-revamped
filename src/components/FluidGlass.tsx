...
export default memo(function FluidGlass({ mode = 'lens', lensProps = {}, barProps = {}, cubeProps = {} }: FluidGlassProps) {
  const Wrapper = useMemo(() => mode === 'bar' ? Bar : mode === 'cube' ? Cube : Lens, [mode]);
  const rawOverrides = useMemo(() => mode === 'bar' ? barProps : mode === 'cube' ? cubeProps : lensProps, [mode, barProps, cubeProps, lensProps]);
